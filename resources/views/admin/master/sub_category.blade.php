...
@section('breadcrumb')
<h3 class="page-title">Sub Category</h1>
@endsection
...
@section('content')
                ...
                <div class="modal fade" tabindex="-1" id="catmodel" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title mt-0" id="myLargeModalLabel">Sub Category From</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                            </div>
                            <div class="modal-body">
                    <!-- <form class="" action="#"> -->
                        {{ Form::open(array('url' => 'sub-category', 'method'=>'post','id'=>'cat_from')) }}
                    ...
                    {{Form::select('category',[''=>'Please Select'] +$category,Null,array('class'=>'form-control','id'=>'category_id'))}}
                 ...
                 <div class="container-fluid">
                        ...
                        <table id="datatable" class="table table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                           ...
                           @foreach($result as $val)     
                            ...
                            <tr>
                                <td>{{$val['name']}}</td>
                                <td>{{$category[$val['category_id_fk']]}}</td>                           
                                <td>{{$val['alias']}}</td>
                                <td>{{$val['created_at']}}</td>
                                <td>{{$val['updated_at']}}</td>    
                                <td>
                                    <button onclick="editfuc('{{$val['name']}}','{{$val['alias']}}','{{$val['id']}}','{{$val['status']}}','{{$val['category_id_fk']}}')" class="listbtn" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit">
                                    <i class="mdi mdi-grease-pencil"></i>
                                </button>
                                {{ Form::open(array('url' => 'sub-category/'.$val['id'], 'method'=>'Delete',)) }}
...
<script type="text/javascript">
    ...
    function editfuc(name,alias,id,status,category_id_fk){
        $('#name_id').val(name);
        $('#name_id').append("<input name='_method' type='hidden' value='put' />");
        $('#alias_id').val(alias);
        $('#Status').val(status);
        $('#category_id').val(category_id_fk);
        $('#cat_from').attr('action','sub-category/'+id);
        $('#catmodel').modal('show');
